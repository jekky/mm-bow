function generate_kmeans_train_test_histogram
...
for cidx = 1 : size(train_frames,2)
    ...
    for lidx = 1:1:length(train_frames{cidx})
        ...
        G(tidx) = 1;
        save([results_path,Categories.Name{cidx},'/',desc_file(train_frames{cidx}(lidx)).name],'G');
...
save(kmeans_train_histogram_file,'train_data');
...
save(kmeans_test_histogram_file,'test_data');